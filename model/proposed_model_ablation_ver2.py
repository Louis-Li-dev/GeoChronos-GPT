...
class SpatioTemporalEncoder(nn.Module):
    ...
    def forward(self, spatial_coords, temporal_features, use_temporal_for_batch=None):
        ...
        Args:
            ...
            use_temporal_for_batch: (batch,) - boolean mask indicating whether to use temporal for each batch element
        """
        ...
        # If user-aware temporal fusion is enabled, selectively apply temporal features
        if use_temporal_for_batch is not None:
            # Create a mask for which samples should use temporal features
            temporal_mask = use_temporal_for_batch.unsqueeze(1).unsqueeze(2)  # (batch, 1, 1)
            temporal_mask = temporal_mask.expand(batch_size, seq_len, self.hidden_dim)  # (batch, seq_len, hidden_dim)
            
            # Zero out temporal embeddings for users that shouldn't use temporal features
...
class AblationSpatioTemporalGPT(nn.Module):
    ...
    def forward(self, input_ids, spatial_coords, temporal_features, attention_mask=None, 
        ...
        """
        Forward pass with optional user-specific POI masking and user-aware temporal fusion.
        
        Args:
            user_poi_masks: (batch_size, vocab_size) tensor where 1 indicates allowed POIs
            use_temporal_for_batch: (batch_size,) tensor indicating whether to use temporal for each user
        """
        # Get GPT embeddings
        gpt_outputs = self.gpt.transformer(input_ids, attention_mask=attention_mask)
        gpt_hidden = gpt_outputs.last_hidden_state
        
        # Get spatio-temporal embeddings with user-aware temporal fusion
        st_embeddings = self.spatio_temporal_encoder(spatial_coords, temporal_features, use_temporal_for_batch)
...
class AblationPOIGPTRecommender:
    ...
    def __init__(self, hidden_dim: int = 768, num_layers: int = 8, 
                 ...
                 user_aware_temporal_fusion: bool = False):
        ...
        # Training temporal feature ranges for user-aware temporal fusion
        self.training_temporal_ranges = {}  # Store min/max values for each temporal feature during training
        
        config_str = f"temporal={use_temporal}, masking={use_personal_masking}, user_aware_temporal={user_aware_temporal_fusion}"
    ...
    def _prepare_data(self, df: pd.DataFrame):
        ...
        # Store all temporal features for range calculation
        ...
        for user_id in tqdm(unique_users):
            ...
            all_temporal_features.append(temp_features)
        ...
        # Calculate training temporal feature ranges for user-aware fusion
        if self.use_temporal and self.user_aware_temporal_fusion and all_temporal_features:
            all_temp_concat = np.concatenate(all_temporal_features, axis=0)
            self.training_temporal_ranges = {
                'hour_min': all_temp_concat[:, 0].min(),
                'hour_max': all_temp_concat[:, 0].max(),
                'day_min': all_temp_concat[:, 1].min(),
                'day_max': all_temp_concat[:, 1].max(),
                'month_min': all_temp_concat[:, 2].min(),
                'month_max': all_temp_concat[:, 2].max(),
                'epoch_min': all_temp_concat[:, 3].min(),
                'epoch_max': all_temp_concat[:, 3].max(),
            }
            print(f"Training temporal ranges: {self.training_temporal_ranges}")
    ...
    def _should_use_temporal_for_user(self, user_temporal_features: np.ndarray) -> bool:
        """
        Determine if temporal features should be used for a specific user based on
        whether their temporal patterns have any overlap with the training data.
        Uses element-wise overlap for hour and day of week (ignoring month).
        """
        if not self.use_temporal or not self.user_aware_temporal_fusion:
            return self.use_temporal
            
        if not hasattr(self, 'training_temporal_ranges') or not self.training_temporal_ranges:
            return self.use_temporal
        
        # Get unique values from user's temporal features
        user_hours = np.unique(user_temporal_features[:, 0])  # Normalized hours [0, 1]
        user_days = np.unique(user_temporal_features[:, 1])   # Normalized days [0, 1]
        
        # Get training ranges
        train_hour_min = self.training_temporal_ranges['hour_min']
        train_hour_max = self.training_temporal_ranges['hour_max']
        train_day_min = self.training_temporal_ranges['day_min']
        train_day_max = self.training_temporal_ranges['day_max']
        
        # Check for any overlap in hours
        hour_overlap = any(
            train_hour_min <= user_hour <= train_hour_max 
            for user_hour in user_hours
        )
        
        # Check for any overlap in days of week
        day_overlap = any(
            train_day_min <= user_day <= train_day_max 
            for user_day in user_days
        )
        
        # Use temporal features if there's overlap in either hour OR day of week
        should_use = hour_overlap or day_overlap
    ...
    def fit(self, train_df: pd.DataFrame, epochs: int = 5, batch_size: int = 16, 
        ...
        for epoch in range(epochs):
            ...
            for batch in pbar:
                ...
                # Create user-aware temporal fusion mask (during training, always use temporal if enabled)
    ...
    def predict(
        ...
        seen during training, using configurable masking and user-aware temporal fusion.

        Args:
            test_df: Optional test dataframe to analyze temporal patterns for user-aware fusion

        Returns:
            Dict[int, List[str]]: Mapping: user_id -> list of predicted category strings
        """
        ...
        # Analyze test data temporal patterns if user-aware temporal fusion is enabled
        user_temporal_decisions = {}
        if self.use_temporal and self.user_aware_temporal_fusion and test_df is not None:
            print("Analyzing temporal patterns in test data for user-aware fusion...")
            ...
            for user_id in test_df['user_id'].unique():
                if user_id in self.user_last_sequences:
                    ...
                    should_use_temporal = self._should_use_temporal_for_user(test_temp_features)
                    user_temporal_decisions[user_id] = should_use_temporal
            
            temporal_usage_stats = np.mean(list(user_temporal_decisions.values()))
            print(f"User-aware temporal fusion: {temporal_usage_stats:.1%} of users will use temporal features")
        ...
        for user_id, seq_info in tqdm(
            ...
            # Create user-aware temporal fusion decision
            use_temporal_for_batch = None
            if self.use_temporal and self.user_aware_temporal_fusion:
                should_use_temporal = user_temporal_decisions.get(user_id, True)  # Default to True if no test data
    ...
    def _generate(self, input_ids, spatial_coords, temporal_features, user_poi_mask, 
        ...
        """Generate new tokens autoregressively with configurable masking and user-aware temporal fusion."""
        ...
        for _ in range(n_tokens):
            ...
            # Get predictions with configurable masking and user-aware temporal fusion
    ...
    def save(self, path: str):
        ...
        config = {
            ...
            'training_temporal_ranges': self.training_temporal_ranges if hasattr(self, 'training_temporal_ranges') else None
        }
    ...
    def load(self, path: str):
        ...
        self.training_temporal_ranges = self._deserialize_temporal_ranges(config.get('training_temporal_ranges', {}))
...
# 1. Full model (temporal + personal masking + user-aware temporal fusion)
...
for name, model in models.items():
    ...
    # Generate predictions (only full model uses test_df for temporal analysis)
    if name == 'full':
