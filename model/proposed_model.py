...
class MaskedSpatioTemporalGPT(nn.Module):
    """GPT model enhanced with spatio-temporal encoding and user-specific masking for POI prediction."""
    ...
    def __init__(self, vocab_size: int, hidden_dim: int = 768, num_layers: int = 12, 
        ...
        self.spatio_temporal_encoder = SpatioTemporalEncoder(hidden_dim)
...
class MaskedPOIGPTRecommender:
    """Advanced POI recommender using GPT with spatio-temporal features and user-specific masking."""
    ...
    def __init__(self, hidden_dim: int = 768, num_layers: int = 8, 
        ...
        print(f"Initialized Masked POI GPT Recommender on {self.device}")
        
    def _prepare_data(self, df: pd.DataFrame):
        """Convert dataframe to sequences and mappings, recording user-specific POI sets."""
        ...
        for user_id in tqdm(unique_users):
            ...
            # Record user's POI set (including padding token 0)
    ...
    def _create_user_poi_mask(self, user_ids: List[int], vocab_size: int) -> torch.Tensor:
        """Create a mask tensor for user-specific POIs."""
    ...
    def fit(self, train_df: pd.DataFrame, epochs: int = 5, batch_size: int = 16, 
            lr: float = 5e-5):
        """Train the model on the training data with user-specific masking."""
        ...
        self.model = MaskedSpatioTemporalGPT(
            vocab_size=vocab_size,
        ...
        for epoch in range(epochs):
            ...
            for batch in pbar:
                ...
                if input_ids.size(1) > 1:
                    ...
                    # Calculate loss (only on valid targets, masked invalid POIs already have -inf logits)
    ...
    def predict(
        ...
        seen during training, using user-specific POI masking.
    ...
    def _generate(self, input_ids, spatial_coords, temporal_features, user_poi_mask, 
        ...
        """Generate new tokens autoregressively with user-specific masking."""
        ...
        for _ in range(n_tokens):
            ...
            # Get predictions with user-specific masking
            logits = self.model(curr_input, curr_coords, curr_temp, 
                              user_poi_masks=user_poi_mask)
            next_token_logits = logits[:, -1, :] / temperature
            
            # Apply softmax and sample (masking already applied in model forward)
            probs = F.softmax(next_token_logits, dim=-1)
            
            # Ensure we don't sample from masked (zero probability) tokens
            # The masking in the model should have already set these to -inf -> 0 probability
    ...
    def save(self, path: str):
        """Save the model, mappings, and user POI sets."""
        ...
        print(f"Masked model saved to {path}")
    
    def load(self, path: str):
        """Load the model, mappings, and user POI sets."""
        # Load config
        ...
        # Load user POI sets
        with open(os.path.join(path, 'user_poi_sets.pkl'), 'rb') as f:
        ...
        print(f"Masked model loaded from {path}")
